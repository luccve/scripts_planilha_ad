...
class ProcessadorArquivo:
    def __init__(self, caminho_arquivo, callback=None):
    ...
    def ler_arquivo(self):
        try:
            tipo = os.path.basename(self.caminho_arquivo).split('.')[-1]
            ...
            self.targets = [coluna for coluna in self.df.columns] #Popula a variavel
          
            if self.callback:
                self.callback(self.targets)

        except FileNotFoundError:
            raise ValueError(f"Arquivo não encontrado: {self.caminho_arquivo}")
    ...
    def exportar_dataframe(self, nome_arquivo_saida):
        ...
        try:
            ...
            raise ValueError(f"Erro ao exportar o DataFrame: {e}")
